...
const CategoryItem = ({ item, plan, courses }) => {
    ...
    const onCardPress = () => {
        navigation.navigate('ClickClass', { videos: item.videos, name: item.title, description: item.description, img: item.img, pdf: item.pdf, url: item.url, courseId: item.courseId, priceId: item.priceId, plan: item.plan, cardId:item.id })
    }
